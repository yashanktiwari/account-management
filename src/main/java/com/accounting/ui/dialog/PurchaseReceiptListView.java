...
public class PurchaseReceiptListView {

    private final PurchaseReceiptDAO dao = new PurchaseReceiptDAO();
    private final ObservableList<PurchaseReceipt> rows = FXCollections.observableArrayList();
    private List<PurchaseReceipt> allData = new java.util.ArrayList<>();
    private TableView<PurchaseReceipt> table;
    ...
    public Parent createContent() {
        ...
        Label heading = new Label("Money Received Receipts");
        ...
        addBtn.setOnAction(e -> {
            new PurchaseReceiptDialog().show(MainApp.getPrimaryStage(), this::loadRows);
        });
        ...
        TableColumn<PurchaseReceipt, Integer> serialCol = new TableColumn<>("S.No");
        ...
        editItem.setOnAction(e -> {
            PurchaseReceipt selected = table.getSelectionModel().getSelectedItem();
            if (selected != null) {
                new PurchaseReceiptDialog(selected).show(MainApp.getPrimaryStage(), this::loadRows);
            }
        });
        ...
        deleteItem.setOnAction(e -> {
            ...
            boolean confirmed = AlertUtil.showConfirmation("Delete Purchase Receipt",
                    "Delete Purchase Receipt '" + selected.getReceiptNo() + "'? This cannot be undone.");
            if (!confirmed) return;
            AppExecutor.submit(() -> {
                try {
                    dao.delete(selected.getId());
                    Platform.runLater(() -> {
                        NotificationUtil.showSuccess("Deleted", "Purchase Receipt deleted.");
        ...
        table.setRowFactory(tv -> {
            TableRow<PurchaseReceipt> row = new TableRow<>();
    ...
    private void loadRows() {
        AppExecutor.submit(() -> {
            try {
                List<PurchaseReceipt> data = dao.getAll();
                Platform.runLater(() -> {
                    allData = data;
                    applyFilters();
                });
    ...
    private void applyFilters() {
        List<PurchaseReceipt> filtered = new java.util.ArrayList<>(allData);
    ...
    private void updateStats() {
        ...
        double total = rows.stream().mapToDouble(PurchaseReceipt::getAmount).sum();
