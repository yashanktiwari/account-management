...
public class PurchaseInvoiceListView {
    ...
    private final PurchaseInvoiceDAO dao = new PurchaseInvoiceDAO();
    private final ObservableList<PurchaseInvoice> rows = FXCollections.observableArrayList();
    private TableView<PurchaseInvoice> table;
    ...
    public Parent createContent() {
        ...
        Label heading = new Label("Purchase Invoice List");
        ...
        addBtn.setOnAction(e -> {
            new PurchaseInvoiceDialog().show(MainApp.getPrimaryStage(), this::loadRows);
        });
        ...
        searchField.textProperty().addListener((obs, oldVal, newVal) -> {
            ...
            debounceTimer.schedule(new TimerTask() {
                @Override
                public void run() {
                    Platform.runLater(PurchaseInvoiceListView.this::searchRows);
                }
            }, DEBOUNCE_DELAY);
        });
        ...
        TableColumn<PurchaseInvoice, Integer> serialCol = new TableColumn<>("S.No");
        ...
        TableColumn<PurchaseInvoice, Double> totalAmountCol = new TableColumn<>("Total Amount");
        totalAmountCol.setCellValueFactory(new PropertyValueFactory<>("netAmount"));
        totalAmountCol.setPrefWidth(120);
        totalAmountCol.setCellFactory(col -> new TableCell<PurchaseInvoice, Double>() {
            @Override
        ...
        table.setRowFactory(tv -> {
            TableRow<PurchaseInvoice> row = new TableRow<>();
        ...
        table.getColumns().addListener((javafx.collections.ListChangeListener<TableColumn<PurchaseInvoice, ?>>) change -> saveColumnState());
    ...
    private void loadRows() {
        AppExecutor.submit(() -> {
            try {
                ...
                List<PurchaseInvoice> data = dao.getAll(startDate, endDate);
                ObservableList<PurchaseInvoice> observableData = FXCollections.observableArrayList(data);
                Platform.runLater(() -> {
    ...
    private void updateStats(ObservableList<PurchaseInvoice> data) {
        rowCountLabel.setText("Total: " + data.size());
        double total = data.stream().mapToDouble(PurchaseInvoice::getNetAmount).sum();
        ...
        AppExecutor.submit(() -> {
            try {
                ...
                if (!searchTerms.isEmpty()) {
                    List<PurchaseInvoice> termResults = dao.searchAllColumns(searchTerms.iterator().next());
                    for (String term : searchTerms) {
                        List<PurchaseInvoice> results = dao.searchAllColumns(term);
                ...
                if (!liveSearchText.isEmpty()) {
                    List<PurchaseInvoice> liveResults = dao.searchAllColumns(liveSearchText);
                    data.retainAll(liveResults);
                }
                
                final List<PurchaseInvoice> finalData = data;
    ...
    private TableColumn<PurchaseInvoice, Object> col(String title, String property, double width) {
        TableColumn<PurchaseInvoice, Object> column = new TableColumn<>(title);
    ...
    private void saveColumnState() {
        Preferences prefs = Preferences.userNodeForPackage(PurchaseInvoiceListView.class);
        StringBuilder columnOrder = new StringBuilder();
        StringBuilder columnWidths = new StringBuilder();
        
        for (int i = 0; i < table.getColumns().size(); i++) {
            TableColumn<PurchaseInvoice, ?> col = table.getColumns().get(i);
            if (i > 0) {
                columnOrder.append(",");
                columnWidths.append(",");
            }
        ...
        prefs.put("purchaseInvoiceTable_columnOrder", columnOrder.toString());
        prefs.put("purchaseInvoiceTable_columnWidths", columnWidths.toString());
    }

    private void loadColumnState() {
        Preferences prefs = Preferences.userNodeForPackage(PurchaseInvoiceListView.class);
        String columnOrderStr = prefs.get("purchaseInvoiceTable_columnOrder", "");
        String columnWidthsStr = prefs.get("purchaseInvoiceTable_columnWidths", "");
        
        try {
            // Restore column order
            if (!columnOrderStr.isEmpty()) {
                String[] columnNames = columnOrderStr.split(",");
                List<TableColumn<PurchaseInvoice, ?>> currentColumns = new java.util.ArrayList<>(table.getColumns());
                ...
                for (int i = 0; i < columnNames.length && i < currentColumns.size(); i++) {
                    ...
                    for (int j = i; j < currentColumns.size(); j++) {
                        if (currentColumns.get(j).getText().equals(targetName)) {
                            // Swap columns
                            TableColumn<PurchaseInvoice, ?> temp = currentColumns.get(i);
